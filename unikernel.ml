...
module Main
  ...
  let write_string flow s =
    S.TCPV4.write flow (Cstruct.of_string s)
  ...
  let start kv s : unit Lwt.t =
    ...
    let rec session_step flow interpreter input : unit Lwt.t =
      ...
      let read_more_input () =
        S.TCPV4.read flow >>= function
          ...
          | Ok (`Data b) ->
              ...
              let input : char array = Array.make (String.length s) ' ' in
              String.iteri (fun i c -> input.(i) <- c) s;
              Lwt.return (Some (Array.to_list input))
      in
      ...
      | Interpreter.Waiting_for_input ->
          begin
            match input with
            | [] ->
              read_more_input () >>= begin function
                | None -> Lwt.return None
                | Some (input) -> Lwt.return (Some (interpreter, input))
              end
            | ch :: tl ->
              Lwt.return (Some (Interpreter.step_with_input interpreter ch, tl))
          end
      | Interpreter.Halted ->
          Lwt.return None
      in
